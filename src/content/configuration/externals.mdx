...
将 externals 类型设置为`'module'`， 它告诉 Webpack 在 module 中为外部引用生成形如 `import * as X from '...'`的代码。

确保首先把 [`experiments.outputModule`](/configuration/experiments/#experimentsoutputmodule) 设置为`true`, 否则 Webpack将会报错。
...
将 externals 类型设置为 `'node-commonjs'`， 它告诉 Webpack 在 module 中为外部引用引入`'module'`的 [`createRequire`](https://nodejs.org/api/module.html#module_module_createrequire_filename) 函数构造一个require函数
...
将 externals 类型设置为 `'script'`， 它告诉 Webpack 通过HTML中的 `<script>` 标签暴露预定义的全局变量的脚本，当脚本被加载完后 `<script>` 标签会被删除。
